...
export function SearchPage() {
  ...
  const typeParam = params.get('typ') || 'alla'
  const origin = typeParam === 'sammanfattning' || typeParam === 'demo' ? 'demo' : typeParam
  ...
  return <section className="search-page page-wrap">
    ...
    <header className="search-hero"><span className="eyebrow">Nyhetsarkiv</span><h1>Hitta en ljusglimt</h1><p>Sök bland svenska källsammanfattningar och källnotiser från det automatiska flödet.</p>
    ...
    <div className="filter-bar"><div><Filter size={17} /><strong>Filtrera</strong></div><div className="filter-scroll" aria-label="Filtrera på kategori">{categories.map((item) => <button type="button" key={item} className={category === item ? 'active' : ''} aria-pressed={category === item} onClick={() => update('kategori', item)}>{item}</button>)}</div><label><SlidersHorizontal size={16} aria-hidden="true" /><span className="sr-only">Filtrera på innehållstyp</span><select value={origin === 'demo' ? 'sammanfattning' : origin} onChange={(e) => update('typ', e.target.value)}><option value="alla">Allt innehåll</option><option value="sammanfattning">Sammanfattningar av källor</option><option value="fetched">Källnotiser</option></select></label></div>
