...
export function ProfilePage() {
  ...
  const { saved, refresh: refreshSaved } = useSaved()
  ...
  if (!user) return <section className="auth-page page-wrap"><div className="auth-copy"><span className="eyebrow">Medlemskap</span><h1>Spara, följ och delta</h1><p>Ett konto gör att du kan bygga en egen läslista och delta i det modererade forumet.</p><ul><li><Bookmark /> Spara nyheter till senare</li><li><Shield /> Skriv i ett förhandsmodererat forum</li><li><UserRound /> Hantera din profil på ett ställe</li></ul></div><div className="auth-card"><div className="auth-tabs"><button className={tab === 'login' ? 'active' : ''} onClick={() => setTab('login')}>Logga in</button><button className={tab === 'register' ? 'active' : ''} onClick={() => setTab('register')}>Skapa konto</button></div><form onSubmit={submit}>{tab === 'register' && <label>Namn<input name="name" minLength={2} maxLength={50} required autoComplete="name" /></label>}<label>E-post<input name="email" type="email" required autoComplete="email" /></label><label>Lösenord<input name="password" type="password" minLength={8} required autoComplete={tab === 'login' ? 'current-password' : 'new-password'} /></label><button className="button primary" type="submit">{tab === 'login' ? 'Logga in' : 'Skapa konto'}</button><p role="status" className="form-status">{status}</p></form><div className="auth-divider"><span>eller</span></div><div id="google-signin" className="google-signin" /><p className="google-note">Google-knappen visas när ett Client ID har konfigurerats. E-post och lösenord fungerar lokalt.</p></div></section>

  return <section className="profile-page page-wrap"><header><div className="profile-avatar">{user.name.slice(0, 1).toLocaleUpperCase('sv')}</div><div><span className="eyebrow">Din profil</span><h1>Hej {user.name}</h1><p>{user.email}</p></div><button className="button ghost" onClick={async () => { await logout(); navigate('/') }}><LogOut size={17} /> Logga ut</button></header><div className="profile-layout"><aside><a href="#sparade"><Bookmark /> Sparade nyheter</a><a href="#installningar"><Settings /> Inställningar</a><Link to="/forum"><UserRound /> Forumet</Link></aside><div><section id="sparade" className="profile-panel"><header><div><span className="eyebrow">Läslista</span><h2>Sparade nyheter</h2></div><span>{saved.length} sparade</span></header>{saved.length ? <div className="saved-list">{saved.map((item) => <article key={item.article_id}><div><span>{item.source}</span><h3>{item.title}</h3><p>{item.summary}</p></div><div><a href={item.url} target="_blank" rel="noreferrer">Öppna källan</a><button onClick={async () => { await fetch(`/api/saved/${encodeURIComponent(item.article_id)}`, { method: 'DELETE', credentials: 'same-origin' }); await refreshSaved() }}>Ta bort</button></div></article>)}</div> : <div className="empty-state"><Bookmark /><h3>Läslistan är tom</h3><p>Spara en nyhet så hamnar den här.</p><Link className="button primary" to="/">Hitta nyheter</Link></div>}</section><ProfileSettings name={user.name} onUpdated={refresh} /></div></div></section>
}
